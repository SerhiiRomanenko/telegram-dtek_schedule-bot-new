...
async function editPost(msgId, caption, image, isBuffer) {
    try {
        let msg;
        if (isBuffer) {
            msg = await bot.sendPhoto(CHAT_ID, image, {
                caption,
                parse_mode: "HTML",
            }, {
                filename: `schedule_${Date.now()}.jpg`,
        ...
        return { newMsgId: msg.message_id };
    } catch (err) {
        if (err.code === "EFATAL" || err.response?.statusCode === 403) {
            return false;
        }
...
async function checkSchedule() {
    ...
    try {
        ...
        // Перший запуск ігнорує збіг хешу, щоб оновити/надіслати пост відразу
        if (!isFirstRun && savedHash === currentHash) {
            return;
        }
        ...
        if (isNewDay) {
            ...
            console.log("📢 Надіслано новий пост при запуску/зміні дня");
        } else {
            const result = await editPost(savedMsgId, caption, image, isBuffer);
            if (!result) {
                await bot.sendMessage(CHAT_ID, `🔄 Графік оновлено станом на ${getKyivTimeShort()}. Актуальна версія нижче.`);
                const msg = await sendPhoto(image, caption, isBuffer);
                await redis.set("dtek:msgId", String(msg.message_id));
                console.log("📢 Старий пост не знайдено, надіслано новий");
            } else {
                await redis.set("dtek:msgId", String(result.newMsgId));
                console.log("🔄 Пост замінено, новий msgId записано");
            }
        }

        await redis.set("dtek:hash", currentHash);
        isFirstRun = false; // Вимикаємо прапорець після першого вдалого проходу
...
// Одразу запускаємо першу перевірку
checkSchedule();

// Налаштовуємо циклічний інтервал
const pollInterval = setInterval(checkSchedule, POLL_INTERVAL_MS);

// Функція плавного завершення роботи
function shutdown() {
    console.log("🛑 Shutting down...");
    clearInterval(pollInterval);
    process.exit(0);
...
// Web-сервер для Render (запобігає засипанню та помилкам Port Binding)
const app = express();
app.get("/", (req, res) => {
    res.send("Бот працює 🚀");
});
